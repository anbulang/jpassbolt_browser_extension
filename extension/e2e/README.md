...
品牌图标回归：先 `npm run build`，再 `npm run e2e:brand`。该测试加载真实构建包，使用离线登录页检查密码框图标从扩展资源加载、严格 `img-src` 策略下仍有可见入口，以及图片失败后的 `JP` 后备标识。无需后端或账号。
...
- **先证通路存活**再断言"0 封"——否则 SMTP 断了和门控生效长得一模一样。canary 显式开启 `send_folder_share`，并实测投递延迟与"0 封"等待窗做倍数比对。
...
- 脚本保存所涉及通知开关的原值，显式设置测试条件，最终恢复原值并校验响应；清理失败也会报告失败（开关持久化在 `organization_settings`，不能依赖出厂默认值）。
- 密文中每一个 PKESK 收件人都必须属于预期用户，两封邮件均不得夹带其他用户的密钥。可选 disabled 用户用例以 `SKIP` 计入完成数，不冒充通过，也不会误报中途中断。
